...
public class pageManagerProduct extends JPanel {

    private static Container Container;

    public pageManagerProduct() {
    ...
    private JPanel panelCenter() {
        ...
        //region // ! [ panelTableProductList ] ! \\

        JPanel panelTableProductList = new JPanel(null);
        panelTableProductList.setLayout(new FlowLayout());
        panelTableProductList.setBackground(new Color(0, 0, 0, 0));
        panelTableProductList.setBounds(50, 100, 800, 440);
        // panelTableProductList.setBorder(BorderFactory.createLineBorder(Color.BLACK, 3));

        String[] columnNames = { "ID", "ชื่อสินค้า", "ราคา", "ที่อยู่รูปภาพ" };
        new managerData();
        Object[][] data = managerData.getDataProduct();
        ...
        JTable tableProductList = new JTable(model);
        tableProductList.setFont(new Font("Tahoma", Font.PLAIN, 16));
        tableProductList.setPreferredScrollableViewportSize(new Dimension(775, 365));
        tableProductList.setBackground(Color.WHITE);
        tableProductList.setBounds(0, 0, 790, 2000);
        tableProductList.setFillsViewportHeight(true);
        tableProductList.setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
        tableProductList.setRowHeight(30);
        tableProductList.setFocusable(false);
        tableProductList.setShowHorizontalLines(false);
        tableProductList.setSelectionBackground(new Color(232, 57, 95));

        tableProductList.getTableHeader().setPreferredSize(new Dimension(50, 50));
        tableProductList.getTableHeader().setReorderingAllowed(false);
        tableProductList.getTableHeader().setForeground(Color.WHITE);
        tableProductList.getTableHeader().setForeground(Color.WHITE);
        tableProductList.getTableHeader().setFont(new Font("Tahoma", Font.BOLD, 18));
        tableProductList.getTableHeader().setBackground(new Color(133, 64, 171));
        tableProductList.getTableHeader().setReorderingAllowed(false);

        tableProductList.getColumnModel().getColumn(0).setPreferredWidth(50);
        tableProductList.getColumnModel().getColumn(1).setPreferredWidth(250);
        tableProductList.getColumnModel().getColumn(2).setPreferredWidth(100);
        tableProductList.getColumnModel().getColumn(3).setPreferredWidth(250);

        DefaultTableCellRenderer alignTextTable = new DefaultTableCellRenderer();
        alignTextTable.setHorizontalAlignment(JLabel.CENTER);
        tableProductList.getColumnModel().getColumn(0).setCellRenderer(alignTextTable);
        tableProductList.getColumnModel().getColumn(1).setCellRenderer(alignTextTable);
        tableProductList.getColumnModel().getColumn(2).setCellRenderer(alignTextTable);
        tableProductList.getColumnModel().getColumn(3).setCellRenderer(alignTextTable);

        JScrollPane scrollPane = new JScrollPane(tableProductList);
        scrollPane.setBorder(BorderFactory.createLineBorder(Color.BLACK, 3));

        panelTableProductList.add(scrollPane);

        //endregion // ! [ panelTableProductList ] ! \\

        //region // ! [ panelAddProduct ] ! \\

        JPanel panelAddProduct = new JPanel(null);
        panelAddProduct.setBounds(900, 100, 650, 600);
        panelAddProduct.setBackground(new Color(0,0,0,0));
        panelAddProduct.setBorder(BorderFactory.createLineBorder(Color.BLACK, 3));

        JLabel labelTitle = new JLabel(" จัดการสินค้า", SwingConstants.CENTER);
        labelTitle.setFont(new Font("Tahoma", Font.BOLD, 32));
        labelTitle.setBounds(0, 0, 650, 100);
        labelTitle.setOpaque(true);
        labelTitle.setBackground(new Color(133, 64, 171));
        labelTitle.setForeground(Color.WHITE);
        labelTitle.setBorder(BorderFactory.createLineBorder(Color.BLACK, 3));

        JLabel labelnameproduct = new JLabel("ชื่อสินค้า ( Product Name )", SwingConstants.LEFT);
        labelnameproduct.setOpaque(false);
        labelnameproduct.setBounds(100, 100, 550, 50);
        labelnameproduct.setFont(new Font("Tahoma", Font.BOLD, 20));

        JTextField FieldProduct = new JTextField(20);
        FieldProduct.setBounds(100, 150, 450, 50);
        FieldProduct.setFont(new Font("Tahoma", Font.BOLD, 20));


        JLabel labelPrice = new JLabel("ราคาสินค้า ( Price )", SwingConstants.LEFT);
        labelPrice.setOpaque(false);
        labelPrice.setBounds(100, 200, 450, 50);
        labelPrice.setFont(new Font("Tahoma", Font.BOLD, 20));

        JTextField FieldPrice = new JTextField(20);
        FieldPrice.setBounds(100, 250, 450, 50);
        FieldPrice.setFont(new Font("Tahoma", Font.BOLD, 20));


        JLabel labelImages = new JLabel("ที่อยู่รูปภาพ ( Path )", SwingConstants.LEFT);
        labelImages.setOpaque(false);
        labelImages.setBounds(100, 300, 450, 50);
        labelImages.setFont(new Font("Tahoma", Font.BOLD, 20));

        JTextField FieldImages = new JTextField(20);
        FieldImages.setBounds(100, 350, 450, 50);
        FieldImages.setFont(new Font("Tahoma", Font.BOLD, 20));
        ...
        btnAdd.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                int rows = tableProductList.getRowCount();

                if (FieldProduct.getText().toString().isEmpty() || FieldPrice.getText().toString().isEmpty()
                        || FieldImages.getText().toString().isEmpty()) {
                    ...
                    String productName = FieldProduct.getText().toString();
                    String productPrice = FieldPrice.getText().toString();
                    String productImgPath = FieldImages.getText().toString();

                    Object[] newRow = { rows+1 , productName, productPrice, productImgPath };
                    model.addRow(newRow);

                    FieldProduct.setText(null);
                    FieldPrice.setText(null);
                    FieldImages.setText(null);
        ...
        panelAddProduct.add(labelTitle);
        panelAddProduct.add(labelnameproduct);
        panelAddProduct.add(FieldProduct);

        panelAddProduct.add(labelPrice);
        panelAddProduct.add(FieldPrice);

        panelAddProduct.add(labelImages);
        panelAddProduct.add(FieldImages);

        panelAddProduct.add(btnAdd);

        //endregion // ! [ panelAddProduct ] ! \\
        ...
        labelTitleControl.setBackground(new Color(133, 64, 171));
        ...
        btnremove.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                if (tableProductList.getSelectedRow() == -1) {
                    ...
                    if (selection == JOptionPane.YES_OPTION) {
                        ...
                        int[] selectedRows = tableProductList.getSelectedRows();
        ...
        btnsave.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (selection == JOptionPane.YES_OPTION) {
                    ...
                    int rows = tableProductList.getRowCount();
                    int cols = tableProductList.getColumnCount();
                    ...
                    managerData.saveProductData(newData);
        ...
        panelCenter.add(panelAddProduct);
        panelCenter.add(panelTableProductList);
